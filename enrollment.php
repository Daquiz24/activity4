...
if ($availableSlotsResult !== false) {
    ...
    if ($availableSlotsCount > 0) {
        // Fetch data from the database with a condition for start time greater than the current time and available slots
        $result = $conn->query("SELECT * FROM appointment WHERE start_time  > NOW() AND slots > 0");
        ...
        if ($result === false) {
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    // Assuming there's an 'id' column in your appointments table
                    echo "<td>";
                    echo "<form action='config/enroll.php' method='post' style='display: inline;'>";
                    echo "<input type='radio' name='selectedAppointment' id='selectedAppointment' value='" . $row["id"] . "' required>";
                ...
                echo "<tr><td colspan='5'>No available appointments with slots. Please check back later.</td></tr>";
            }
        }
    } else {
        echo "<tr><td colspan='5'><b>There is no Appointment set. Wait for the Admin to set an Appointment. Thank you!</b></td></tr>";
    }
} else {
    // Display an error message
    echo "Error: " . $conn->error;
...
?>
 ...
 <div class="enrollment-container">
    ...
    <input type="text" name="sformat" value="@student" readonly>
        ...
        <input type="text" id="middleName" name="middleName" placeholder="Middle Name" required><br>
        ...
        <label for="course">Section:</label><br>
        <select id="section" name="section" required><br>
            <option value="choose course">Choose Section:</option>
